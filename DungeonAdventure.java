...
public class DungeonAdventure {
    ...
    public static int computeAttackDamage(int strength, int weaponDie, int armor, Random rng) {
        ...
        int roll = rollDice(1, weaponDie, rng);
        int dmg = roll + strength - armor;
        if (dmg < 0) dmg = 0;
    ...
    public static int distanceBetween(int x1, int y1, int x2, int y2) {
        ...
        return Math.max(dx, dy); // <-- replace this
    }
    ...
    public static boolean canReach(int cx, int cy, int tx, int ty, int speed) {
        ...
        return distanceBetween(cx, cy, tx, ty) <= speed; // <-- replace this
    }
    ...
    public static String describeAttack(String attackerName, int attackerType,
        ...
        String typeName = TYPE_NAMES[attackerType];

        if (damage == 0) {
            return attackerName + " the " + typeName + " attacks " +
                   defenderName + " but the attack bounces off!";
        }

        return attackerName + " the " + typeName + " attacks " +
               defenderName + " for " + damage + " damage!"; // <-- replace this
    }
    ...
    public static String generateEncounter(int partyLevel, Random rng) {
        ...
        String monster = monsters[roll - 1];

        return "A level " + partyLevel + " party encounters a " + monster + "!"; // <-- replace this
    }
